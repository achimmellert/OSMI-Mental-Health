...
def cluster_plot(X, labels):
    pairs = list(combinations(range(X.shape[1]), 2))[:12]  # nur die ersten 12 Kombinationen
    n_plots = len(pairs)
    cols = 4
    ...
    plt.savefig("../Feature Engineering/Plots mit Feature Engineering/Zwölf Komponenten kombiniert.png", dpi=300)
    plt.show()
...
def silhoutte_plot(model, X):
    ...
    plt.savefig("../Feature Engineering/Plots mit Feature Engineering/Silhoutte Plot.png", dpi=300)
    plt.show()


def main():

    filepath = Path.cwd().parent / "Feature Engineering" / "df_mit_featuretools.pkl"
    df = load_data(filepath)

    # Starte MLflow Run
    with mlflow.start_run(run_name="UMAP + KMeans Clustering + featuretools"):
        ...
        mlflow.log_artifact("../Feature Engineering/Plots mit Feature Engineering/Zwölf Komponenten kombiniert.png")

        # 6. Silhouette-Plot
        silhoutte_plot(kmeans, X_reduced)
        mlflow.log_artifact("../Feature Engineering/Plots mit Feature Engineering/Silhoutte Plot.png")
